...
const SelectUserRolesRepartidores = ({
  inputState,
  validationState,
  withLabel = true,
  autoFocus = false,
  ...
  const loadList = async () => {
    User.getRoles((roles, response) => {
      // Filtrar solo los roles con ID 5 y 6
      const filteredRoles = roles.filter(role => 
        role.idRol === 5 || role.idRol === 6
      );
      setSelectList(filteredRoles);
    }, (error) => {
      console.log(error);
    })
  }
...
export default SelectUserRolesRepartidores;
